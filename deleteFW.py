...
del_statment = ("DELETE FROM {} WHERE (fwid) IN ((\'{}\'))")
get_fw_statemnet = ("SELECT fwid FROM fwinfo WHERE fwid=\'{}\'")
...
def del_all(db: 'db object', fw_name: str) -> None:
    try:
        ...
        for [vd] in db:
            for k in ('vlan', 'fwinfo', 'application'): 
                print(f'Executing sql command: \"{del_statment.format(k, fw_name)}\"')
                db.execute(del_statment.format(k, fw_name))
